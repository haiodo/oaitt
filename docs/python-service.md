...
и на 6-9% быстрее fp16 (146x против 138x на 137s; на коротких чанках Golos тот же порядок).
Дефолт — `fp16`.

Длинное аудио режется по паузам на куски до `PARAKEET_CHUNK_SEC` (20s), как у GigaAM.
Окна `parakeet-mlx` по 120s с перекрытием на митингах теряли речь: на части окон модель
отдаёт пустой результат, на часовой записи пропало ~140s подряд. Аудио идёт в модель
массивом, ffmpeg не нужен.
