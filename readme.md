...
- Поднимает OpenAI-совместимый сервис на порту 9007 - тот же порт и тот же токен `key`,
  что у Python-версии, так что это drop-in замена без правки конфигов.
- Держит воркеры отдельными процессами и перезапускает упавшие. Один воркер обслуживает
  порт напрямую, несколько - через встроенный балансировщик.
- Показывает живую статистику: запросы, скорость, p95, график запросов по минутам, CPU и
  память каждого воркера.
- Пишет лог запросов с ротацией 7 дней, открывается кнопкой из меню.
- Умеет копить датасет из аудио и расшифровок для дообучения - по умолчанию выключено,
  лимит 10 ГБ.

Настройки - три вкладки: сервер (воркеры, порты, токен), модели (какие держать, докачка,
выгрузка по простою), данные (статистика, датасет, логи).

Почему воркеры именно процессами: MLX не масштабируется внутри одного процесса. Пул копий
модели там не даёт ничего (143x против 144x у одной копии), рост идёт только процессами -
144x, 263x, 369x на одном, двух и четырёх.

## Документация

Указатель - [`docs/INDEX.md`](docs/INDEX.md).

- [`docs/swift-port.md`](docs/swift-port.md) - нативный Swift/MLX порт
- [`docs/benchmarks.md`](docs/benchmarks.md) - замеры и качество
